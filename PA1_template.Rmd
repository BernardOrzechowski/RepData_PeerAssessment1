...
We will fill the missing steps values using the mean for that 5-minute interval. We will use the round function on the means to round the missing values to the nearest integer (hence it doesnt't make sense to have i.e. 5.4 steps for a day)
...
```{r makeHistogramWithoutNA, echo=TRUE}
library(ggplot2)

qplot(sumOfSteps, data = dayAggregateseWithoutNA,xlab = "number of steps taken each day (NA replaced with rounded means)",ylab = "count", main = "Histogram of the total number of steps taken each day ")
...
First we will nedd a 5-minute interval and the average number of steps taken, averaged across all days, with previously NA values replaced by mean rounded t nearest integer. 
...
First we claculate the data. We agregate the data by interval and dayType (weekend or weekday) and calculate the mean for each group.
